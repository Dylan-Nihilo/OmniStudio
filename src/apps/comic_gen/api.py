...
# Media is written under the process working directory by the pipeline. Keep
# the serving root aligned with that runtime directory while allowing explicit
# deployments and tests to override it without exposing an arbitrary path.
def _get_media_project_root() -> Path:
    configured_root = os.getenv("OMNI_STUDIO_MEDIA_PROJECT_ROOT")
    return Path(configured_root).expanduser().resolve() if configured_root else Path.cwd().resolve()


MEDIA_PROJECT_ROOT = _get_media_project_root()
_GENERATED_CACHE_PREFIXES = (
    "cache/voice_preview/",
    "cache/voice_design_preview/",
)
...
# ~/.omni-studio/config.json.  Some read-only packaged/test environments cannot
...
try:
    ...
    fallback_env.setdefault("OMNI_STUDIO_AUTH_SIGNING_SECRET", secrets.token_urlsafe(32))
...
def get_user_config_path() -> str:
    ...
    - Packaged app mode: Uses ~/.omni-studio/config.json
    """
