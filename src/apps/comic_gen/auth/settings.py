...
class AuthSettings:
    ...
    issuer: str = "omni_studio"
    audience: str = "omni-studio"
        ...
        *,
        ...
        if not secret:
            if engine is not None and _count_users(engine) > 0:
                raise RuntimeError(
                    "OMNI_STUDIO_AUTH_SIGNING_SECRET is missing while the database already has users"
                )
        ...
        if (
            ...
            raise ValueError("OMNI_STUDIO_PASSWORD_RESET_TOKEN must contain at least 32 bytes")

        access_ttl = _int_env(env, "OMNI_STUDIO_AUTH_ACCESS_TTL_SECONDS", 900)
        refresh_ttl = _int_env(env, "OMNI_STUDIO_AUTH_REFRESH_TTL_SECONDS", 1209600)
        if not 900 <= access_ttl <= 1800:
            raise ValueError("OMNI_STUDIO_AUTH_ACCESS_TTL_SECONDS must be between 900 and 1800")
        ...
        if env_origins:
            ...
            # OMNI_STUDIO_AUTH_ALLOWED_ORIGINS explicitly.
...
def _read_config(path: Path) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Could not read Omni Studio config: {path}") from exc
    if not isinstance(value, dict):
        raise RuntimeError(f"Omni Studio config must be a JSON object: {path}")
    return value
