...
- Omni Studio ≠ "最好的剧本写作工具"（那是 Laper 的目标）
- Omni Studio = "唯一能让剧本自动变成视频的编辑器"
...
Laper 的"CJK 切换"是二元开关。Omni Studio 将其**升级为两维正交系统**：
...
| FDX Element | Omni Studio Node | 说明 |
|-------------|-------------|------|
...
| 2026-07-03 | 角色输入 = @呼出为主 + 冒号触发 + Tab兼容 | 与 Omni Studio 视频生成流程一致的 @ 交互 |
