...
describe("describePriceItem", () => {
    it("names generation models the way the model picker does", () => {
        expect(describePriceItem({ model_id: "seedance/seedance-2.0-video#r2v", stage: "video" })).toEqual({
            name: "Seedance 2.0 R2V", capability: "r2v", selectable: true, planned: false, retired: false, family: "seedance",
        });
    ...
    it("tells apart awaiting-wiring, retired and missing-entirely models", () => {
        ...
        expect(planned).toMatchObject({ name: "Seedance 2.5 I2V", selectable: false, planned: true, retired: false });
    ...
    it("falls back to the model line when only the mode suffix is unknown", () => {
        ...
        expect(described.name).toBe("Seedance 2.0");
    });
...
describe("catalogModesForStage", () => {
    it("lists only models a user can actually pick", () => {
        ...
        expect(ids).not.toContain("seedance/seedance-2.5-video#i2v");
