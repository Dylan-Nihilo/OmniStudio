...
export default function ModeSelector() {
  ...
  return <nav className={styles.navigation} aria-label={t('compose.modeLabel')}>
    {GROUPS.map(group => <div key={group.label} className={styles.modeGroup}>
      ...
      <div className={navigationStyles.subnavItems}>{group.modes.map(key => <Button key={key} variant="quiet" aria-pressed={mode === key} onPress={() => setMode(key)}>{t(`mode.${key}`)}</Button>)}</div>
    </div>)}
